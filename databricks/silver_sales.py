# =============================================================================
# silver_sales.py
# Databricks Lakehouse: Cleansed & Deduplicated Entity (PySpark)
#
# Pattern: Bronze Delta -> Spark Streaming -> Silver Delta (SCD Type 1)
#
# Target Table: catalog.silver.sales_orders (Managed Unity Catalog Table)
...
# -----------------------------------------------------------------------------
SOURCE_TABLE = f"{CATALOG}.bronze.sales_orders"
TARGET_TABLE = f"{CATALOG}.silver.sales_orders"
DLQ_TABLE = f"{CATALOG}.silver.sales_orders_dlq"
CHECKPOINT_PATH = f"dbfs:/pipelines/checkpoints/{CATALOG}/silver_sales_orders"
...
def create_silver_tables(spark: SparkSession) -> None:
    ...
    in Unity Catalog with Liquid Clustering and appropriate metadata tags.
...
def read_bronze_stream(spark: SparkSession) -> DataFrame:
    """
    Reads the append-only stream from the Bronze Delta table.
...
def process_micro_batch(batch_df: DataFrame, batch_id: int) -> None:
    ...
    allocated_cus = 1.0  # Estimated DBU rate for streaming cleansing node
    ...
    try:
        ...
        # Log success metrics (including the DLQ count as invalid count/deleted)
        log_metrics(
            spark=batch_df.sparkSession,
            job_name="databricks_silver_sales_cleansing",
            batch_id=batch_id,
...
if __name__ == "__main__":
    ...
    spark_session = get_spark_session("databricks_silver_sales", max_executors=3)
